...
::note
L'assistant s'appuie sur [Vercel AI Gateway](https://vercel.com/docs/ai-gateway). Utilisez une **`AI_GATEWAY_API_KEY`** (tableau de bord ou `.env`), **ou** l’OIDC : sur Vercel, **`VERCEL_OIDC_TOKEN`** est injecté pour vous—aucune **`VERCEL_OIDC_TOKEN`** (ni clé API) à saisir dans Projet → Variables d’environnement pour ce mode. En local : **`vercel env pull`** sur un projet lié pour remplir `.env`.
::
...
### 1. S'authentifier auprès d'AI Gateway

Choisissez **une** des options :

- **Clé API** — Créez une clé dans [Vercel AI Gateway](https://vercel.com/~/ai/api-keys). AI Gateway route les requêtes vers OpenAI, Anthropic, Google et d'autres fournisseurs via une API unifiée.
- **OIDC** — Sur Vercel, **`VERCEL_OIDC_TOKEN`** est injecté automatiquement : rien à créer ni à coller dans le tableau de bord pour l’auth AI Gateway. En [local](https://vercel.com/docs/cli/env), sur un [projet lié](https://vercel.com/docs/cli/link), lancez **`vercel env pull`** pour mettre le jeton dans votre `.env`.

### 2. Variables d'environnement

**Clé API** — Ajoutez **`AI_GATEWAY_API_KEY`** dans le tableau de bord Vercel (ou dans `.env` en local) :

```bash [.env]
AI_GATEWAY_API_KEY=votre-cle-api
```

**OIDC** — N’ajoutez **pas** **`VERCEL_OIDC_TOKEN`** à la main dans l’interface Vercel ; il est géré automatiquement en production et en preview. Un fichier `.env` avec ce jeton sert seulement après **`vercel env pull`** pour le build local.

### 3. Déployer

L'assistant s'active si **`AI_GATEWAY_API_KEY`** ou **`VERCEL_OIDC_TOKEN`** est disponible au build (sur Vercel, l’OIDC suffit sans variable manuelle). Déployez votre site et l'assistant sera disponible.
...
L'assistant est désactivé si ni **`AI_GATEWAY_API_KEY`** ni **`VERCEL_OIDC_TOKEN`** n'est défini. Retirez ou commentez les deux si vous utilisez les deux mécanismes :
