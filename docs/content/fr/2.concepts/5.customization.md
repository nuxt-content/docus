...
Lorsque la [`subNavigation`](/fr/concepts/configuration#sous-navigation) est activée, Docus affiche une barre de navigation secondaire sous l'en-tête. Pour ajouter du contenu personnalisé à droite de cette barre (ex. un toggle, un badge), créez un composant `components/AppHeaderBottomRight.vue`.
