...
Docus's [Assistant](/en/ai/assistant) and [Review Agent](/en/ai/review) both use the built-in MCP server to understand your documentation content. 

You can configure the MCP server used by these agents in `nuxt.config.ts`:

```ts [nuxt.config.ts]
export default defineNuxtConfig({
  docus: {
    agent: {
      // Connect agents to a specific MCP server
      mcpServer: '/mcp'
    }
  }
})
