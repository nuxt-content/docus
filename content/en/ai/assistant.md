...
Use the keyboard shortcut <kbd value="meta">
...
The assistant is automatically enabled when an API key is detected. To explicitly disable it, you can either remove the environment variable or disable it in your `nuxt.config.ts`:
...
Configure advanced options in `nuxt.config.ts` using the `docus.agent` key:
...
<tbody>
  <tr>
    ...
    <td>
      Whether the assistant is enabled (API key present)
    </td>
  </tr>
